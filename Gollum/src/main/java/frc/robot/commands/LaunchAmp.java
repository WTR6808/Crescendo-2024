...
public class LaunchAmp extends Command {
  ...
  public LaunchAmp(IT_IS_A_LAUNCHER launcher) {
    m_launcher = launcher;
  ...
  public void initialize() {
    m_timer = 0;
    m_launcher.launchAmp();
  }

  // Called every time the scheduler runs while the command is scheduled.
  @Override
  public void execute() {
    if(Math.abs(m_launcher.getVelocity()-Constants.launcherConstants.AMP_VELOCITY) <= Constants.launcherConstants.AMP_VELOCITY_TOLERANCE && m_timer<=0){
